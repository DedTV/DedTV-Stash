...
# Default: # {$studio}.{$date}.{$performer}.{$title}
...
studio_templates = {
#"Compilation": "{($studio).}{[$date].}{'$performer'.}$title",
...
default_template = "{($studio).}{$date.}{$performer.}{$title} {[$tags]}"
# {($studio).}{[$date].}{'$performer'.}$title
...
p_tag_templates = {
"Performer Sort": r"F:\Unsorted\!Performers\$performer",
"!Unidentified": r"F:\Unsorted\$studio",
"Tube Compilation": r"F:\Unsorted\!Compilations",
}


p_studio_templates = {
"Compilation": r"F:\Unsorted\!Compilations",
}
...
p_default_template = r"F:\Stashed\$studio"
...
performer_ignoreGender = "NON_BINARY" "MALE"
...
order_field = ["$video_codec", "$audio_codec", "$resolution", "tags", "rating", "$height", "$studio_family", "$performer", "$studio", "$parent_studio"]
